  ...
  <main class="main-content position-relative max-height-vh-100 h-100 mt-1 border-radius-lg ">
    ...
    <div class="d-sm-flex align-items-center   justify-content-between border-bottom">
     <ul class="nav nav-tabs" role="tablist">
      ...
      <li class="nav-item">
        <a class="nav-link  " data-bs-toggle="" href="consult_registration" style=" font-size:14px;" role="tab"  aria-selected="true" >Consult Registration</a>
      </li>
    </ul>
  </div><br>
  <div class="row">
         ...
         <table id="example" class="display table" style="width:100%;">
           <thead style=" background-color:#e600ac; color:white;">
            <tr>
              ...
              <th style="text-align:left; font-size:12px;">Pre Consultation</th>
           


              

              <th style="text-align:left; font-size:12px;">Doctor</th>
          ...
          <tbody>
            ...
            $query = mysqli_query($q,"select * from nursing_station_pending where doctor='$doctor_name' ");
            $i=1;
            while($doc = mysqli_fetch_assoc($query))
            {
              ...
              echo "<td>" . $doc['Sex'] . "</td>";
              echo "<td>" . $doc['Village'] . "</td>";
              echo "<td>" . $doc['Subcenter'] . "</td>";
              echo "<td>" . $doc['consultation'] . "</td>";

              echo "<td>" . $doc['doctor'] . "</td>";
              if($doc['doctore_status']=='Pending')
              {
                echo "<td><a href='doctor_Pending_.php?id={$doc['id']}&name={$doc['Name']}&mobile={$doc['Mobile']}&village={$doc['Village']}&sex={$doc['Sex']}&subvillage={$doc['Subcenter']}'  class='btn btn-warning' style='height: 32px; background-color: white; color:#e600ac;' >{$doc['doctore_status']}</a></td>";
              }
              else
              {
                echo "<td><a href='#'  style='height: 32px;  background-color: #e600ac; color:white;'class='btn btn-success'>Complate</a></td>";
              }
              ?>
              <td>  
                ...
                <a class="btn btn-primary" href="delete.php?id=<?php echo $row['id']; ?>&status=0" style="height: 32px; color:white;"><i class="mdi mdi-delete"></i></a>
              </td>
              <?php
              $i++;        
            }
