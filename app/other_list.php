         ...
         <div class="form-check">
          <div class="row">
            <div class="col-md-12">
              ...
              <button type="button" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#dept" data-bs-whatever="@getbootstrap"  style="background-color:#e600ac; height: 34px; color:white; float: right; ">Add Other</button>
              <div class="modal fade" id="dept" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="dept">Add Other</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body form-group">
                      <form>
                        <div class="mb-3">
                          <label for="Symtoms-name" class="col-form-label">Other List</label>
                          <input type="text" class="form-control" name="other"id="Symtoms-name">
                        </div>
                      </div>
                      <div class="modal-footer">
                        <input type="reset" class="btn btn-secondary" value="Close"data-bs-dismiss="modal">
                        <input type="submit" class="btn btn-primary" name="add_other" value="Add Other">
                      </div>
                    </form>
                  </div>
                </div>
              ...
              if(isset($_POST['add_other']))
              {
                ...
                $name=$_POST['other'];
                $qry="INSERT INTO `other_list`VALUES ('','$name')";
                $res=mysqli_query($q,$qry);
                if($res)
                {
                 echo '<script>alert("other add successfully");window.location="other_list";</script>';
               }
             }
             ?>
     ...
     <table id="example" class="display table" style="width:100%;">
       <thead style=" background-color:#e600ac; color:white;">
        <tr>
          ...
          <th style="text-align:left; font-size:12px;">Other List</th>
      ...
      <tbody>
        <?php 
        include("../databases/config.php");
        $sql = mysqli_query($q,"select * from other_list");
        ...
        while ($dd = mysqli_fetch_array($sql))
        {
          ...
          <td>  
           ...
           style='height: 32px; background-color: white; color:#8600b3;'><i class="mdi mdi-pencil"></i></button>
            ...
            <a class="btn btn-primary" href="delete.php?id=<?php echo $dd[0]?>&page=other_list" style="height: 32px; color:white;"><i class="mdi mdi-delete"></i></a>
          </td>
          <?php
          echo "</tr>";
...
if(isset($_POST["btn_update"]))
{
   ...
   $b = "UPDATE other_list SET other_name='$name' WHERE id='$id' ";
   $n = mysqli_query($q,$b);
   if($n)
   {
   echo "<script>window.location='other_list.php';</script>";
   }
}
