       ...
       <div class="table-responsive">
        <form method="POST">
         <div class="form-check">
          <div class="row">
            <div class="col-md-12">
             <button type="button" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#subvillage" data-bs-whatever="@getbootstrap"  style="background-color:#e600ac; height: 34px; color:white; float: right; ">Add Village</button>
             <div class="modal fade" id="subvillage" tabindex="-1" aria-labelledby="subvillage" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="subvillage">Add Sub Village</h5>
                  ...
                  <div class="modal-body">
                    <form method="POST">
                      <div class="mb-3">
                        ...
                        <select class="form-select form-select-sm" aria-label=".form-select-sm example" name="vlc">
                          <option selected>Select Village</option>

                          <?php
                          include('../databases/config.php');
                          $s = mysqli_query($q,"select * from country_tb");
                      ...
                      <div class="mb-3">
                        <label for="Village-name" class="col-form-label">Sub Village </label>
                        <input type="text" class="form-control" name="subvlg" id="Village-name">
                      </div>

                    </div>
                    <div class="modal-footer">
                      <input type="reset" class="btn btn-secondary" value="Close"data-bs-dismiss="modal">
                      <input type="submit" class="btn btn-primary" name="add_subvillage" value="Add Sub village">
                    </div>
                  </form>
                  <?php
                  if(isset($_POST['add_subvillage']))
                  {
                    include('../databases/config.php');
                    $namee=$_POST['vlc'];
                    $subvlg = $_POST['subvlg'];
                    $qry="INSERT INTO `state_tb`VALUES ('..','".$subvlg."','".$namee."')";
                    $res=mysqli_query($q,$qry);
                    if($res)
                    {
                     echo '<script>alert("sub village add successfully");window.location="user";</script>';
                   }
                 }
                 ?>
...
<table id="example" class="display table" style="width:100%;">
 <thead style=" background-color:#e600ac; color:white;">
  <tr>
   ...
   <th style="text-align:left; font-size:12px;">HWC ID</th>
...
<tbody >
  <?php 
  include("../databases/config.php");
  $sql = mysqli_query($q,"select * from state_tb");
  ...
  while ($dd = mysqli_fetch_array($sql))
  {
    ...
    <td>  
      ...
      <a class="btn btn-primary" href="delete?id=<?php echo $dd[0]; ?>&page=user_villagelist " style="height: 32px; color:white;"><i class="mdi mdi-delete"></i></a>
    </td>
    <?php
    echo "</tr>";
  ...
  ?>
    ...
    if(isset($_POST['btn_update']))
    {
        ...
        $u_qry = mysqli_query($q,"update state_tb set sname='$u_name' where sid='$u_id' ");
        if($u_qry)
        {
